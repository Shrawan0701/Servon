...
export default function OrdersScreen() {
  ...
  const buildBillHtml = useCallback(
    (tableOrders, discount) => {
      ...
      const merchantName = encodeURIComponent(profile?.business_name || 'Restaurant');
        ...
        <html>
          ...
          <body>
            ...
            <div class="upi-section">
              ...
              <div class="upi-amount">₹${grandTotal.toFixed(2)}</div>

              ${upiId ? `
                <div class="upi-qr-wrapper">
                  <img src="${qrCodeUrl}" alt="UPI Payment QR"
                       onerror="this.src='${qrCodeUrlFallback}';"
                  />
                </div>
                <div class="upi-id">UPI ID: ${upiId}</div>
                <div class="upi-instruction">Scan using Google Pay, PhonePe, Paytm, or any UPI app</div>

                
                </div>
              ` : `
                <div class="upi-error">UPI ID not configured. Please contact restaurant.</div>
              `}
            </div>
