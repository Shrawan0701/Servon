...
const endDate = business.subscription_end_date ? new Date(business.subscription_end_date) : 
null; 
const now = new Date(); 
...
<Text style={styles.bannerText}>Your account is inactive. Activate Servon to start receiving 
orders.</Text>
  ...
  if (status === "ACTIVE" && daysLeft <= 5) { 
    return ( 
      <View style={[styles.banner, { backgroundColor: "#fd7e14" }]}> 
        <Text style={styles.bannerText}>Subscription expires in {daysLeft} day{daysLeft !== 1 ? "s" : 
""}.</Text> 
        <TouchableOpacity onPress={() => navigation.navigate("Subscription")}> 
          <Text style={styles.bannerAction}>Renew →</Text> 
        </TouchableOpacity> 
